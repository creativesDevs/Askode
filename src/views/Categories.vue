...
<section class="flex flex-col gap-14 pt-20 items-center w-full h-screen overflow-hidden">
    ...
    <div class="w-[80vw] min-h-[300px] bg-black/70 p-10 z-50 rounded-lg flex flex-col justify-center relative">
        ...
        <button :disabled="!category" @click="startGame" :class="{ disabled: !category }">
                Press Start
            </button>
    </div>
    <!-- Buttons modal -->
    <div class="z-10 w-[70vw]">
        <ul class="grid grid-cols-2 z-10 gap-10">
            <li class="flex" v-for="category in categories" :key="category.id">
                <button class="py-5 px-5 rounded-xl border-2 border-white flex-grow flex items-center duration-300" @click="selectCategory(category.name)">
